...
<body>
	...
	<div class="page-header">
  		<h1>제품정보수정<small>&nbsp;제품의 정보를 수정하세요.</small></h1>
	</div>
	<form action="${ctx}/coffee/modify.do" method="post" id="form" onsubmit="validateCoffee(); return false;")>
		<div class="form-group">
		<label for="name">제품명</label>
		<input class ="form-control" type="text" name="name" id="name"  value="${coffee.name}"/>
		</div>		
		<div class="form-group">
		<label for="price">가격</label>
		<input class ="form-control" type="text" name="price" id="price" value="${coffee.price}"/>
		</div>
		<div class="form-group">		
		<label for="productInfo">제품설명</label>
		<input type="text" class ="form-control" name="productInfo" id="productInfo" value="${coffee.productInfo}">
		</div>
		<input type="hidden" name="id" value="${coffee.id}"/>
		<input class="btn btn-default orderBtn" type="submit" value="수정"/>
	</form>
	</div>
